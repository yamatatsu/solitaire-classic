...
describe('Foundation Component', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should have proper accessibility attributes', () => {
      ...
      expect(foundation).toHaveAttribute('aria-label', 'Foundation piles - build suits from Ace to King');

      // Check pile accessibility
      const heartsPile = screen.getByTestId('foundation-pile-0');
      expect(heartsPile).toHaveAttribute('role', 'button');
      expect(heartsPile).toHaveAttribute('aria-label', 'hearts foundation pile, 2 cards');

      const emptyPile = screen.getByTestId('foundation-pile-2');
      expect(emptyPile).toHaveAttribute('aria-label', 'clubs foundation pile, 0 cards');
    });
  });
  ...
  describe('Interactions', () => {
    it('should call onFoundationClick when empty foundation is clicked', () => {
      ...
      render(<Foundation foundations={mockFoundations} onFoundationClick={onFoundationClick} />);
    ...
    it('should call onCardClick when foundation card is clicked', () => {
      ...
      render(<Foundation foundations={mockFoundations} onCardClick={onCardClick} />);
  ...
  describe('Card Display', () => {
    ...
    it('should position cards absolutely within piles', () => {
      ...
      const foundationCard = screen.getByTestId('foundation-card-0');
      expect(foundationCard).toHaveClass('absolute', 'inset-0', 'w-full', 'h-full');
    });
  });
  ...
  describe('Custom Props', () => {
    it('should apply custom className', () => {
      render(<Foundation foundations={mockFoundations} className="custom-foundation" />);
    ...
    it('should accept custom data-testid', () => {
      render(<Foundation foundations={mockFoundations} data-testid="custom-foundation" />);
  ...
  describe('Edge Cases', () => {
    it('should handle all empty foundations', () => {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        expect(screen.queryByTestId(`foundation-card-${i}`)).not.toBeInTheDocument();
      }
    });
    ...
    it('should handle foundations with single cards', () => {
      ...
      const onCardClick = vi.fn();
      render(<Foundation foundations={singleCardFoundations} onCardClick={onCardClick} />);
