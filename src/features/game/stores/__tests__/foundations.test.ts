...
import { foundationsAtom, addCardToFoundation, removeCardFromFoundation } from '../foundations';
...
describe('Foundations Atom', () => {
  ...
  describe('Initial State', () => {
    it('should initialize with empty 4-foundation piles', () => {
      ...
      expect(foundations.every((foundation: Card[]) => Array.isArray(foundation) && foundation.length === 0)).toBe(true);
    });
  });
  ...
  describe('Add Card to Foundation Action', () => {
    ...
    it('should reject face-down cards', () => {
      ...
      expect(() => {
        store.set(addCardToFoundation, { foundationIndex: 0, card: faceDownAce });
  ...
  describe('State Validation', () => {
    it('should validate foundations structure', () => {
      ...
      expect(foundations).toHaveLength(4);
      expect(foundations.every((foundation: any) => Array.isArray(foundation))).toBe(true);
    });

    it('should handle complex foundations state', () => {
      const complexFoundations = [
        [createCard('hearts', 1), createCard('hearts', 2), createCard('hearts', 3)],
    ...
    it('should maintain proper suit sequences in foundations', () => {
      ...
      expect(foundations[0].every((card: Card) => card.suit === 'hearts')).toBe(true);
