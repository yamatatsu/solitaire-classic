...
describe("Waste Component", () => {
  ...
  describe("Rendering with Cards", () => {
    ...
    it("should apply proper left offset for card layering", () => {
      ...
      const cardElements = wasteArea.querySelectorAll(
        '[data-testid^="waste-card-"]'
      );
  ...
  describe("Card Interactions", () => {
    ...
    it("should calculate correct card index for onCardClick", () => {
      const onCardClick = vi.fn();
      render(
        <Waste cards={mockCards} maxVisible={2} onCardClick={onCardClick} />
      );
  ...
  describe("Card Display Properties", () => {
    it("should render all visible cards as face up", () => {
      const faceDownCards = mockCards.map((card) => ({
        ...card,
        faceUp: false,
      }));
      ...
      cardElements.forEach((card) => {
        expect(card).toHaveAttribute(
          "data-testid",
          expect.stringMatching(/waste-card-/)
        );
        expect(card).toHaveAttribute(
          "data-testid",
          expect.not.stringMatching(/card-back/)
        );
      });
    });
  ...
  describe("Accessibility", () => {
    it("should have proper aria-label with card counts", () => {
      ...
      expect(waste).toHaveAttribute(
        "aria-label",
        "Waste pile with 5 cards, showing top 3"
      );
    });
    ...
    it("should update aria-label with custom maxVisible", () => {
      ...
      expect(waste).toHaveAttribute(
        "aria-label",
        "Waste pile with 5 cards, showing top 2"
      );
    });
  ...
  describe("Edge Cases", () => {
    it("should handle single card", () => {
      ...
      ).not.toBeInTheDocument(); // No count indicator
    });
