...
import { tableauAtom, addCardToTableauColumn, removeCardFromTableauColumn, flipCardInTableau } from '../tableau';
...
describe('Tableau Atom', () => {
  ...
  describe('Initial State', () => {
    it('should initialize with empty 7-column tableau', () => {
      ...
      expect(tableau.every((column: Card[]) => Array.isArray(column) && column.length === 0)).toBe(true);
    });
  });
  ...
  describe('Remove Card from Column Action', () => {
    it('should remove a card from specified column', () => {
      ...
      store.set(tableauAtom, initialTableau);
      store.set(removeCardFromTableauColumn, { columnIndex: 0, cardId: card2.id });
      const tableau = store.get(tableauAtom);
    ...
    it('should throw error for invalid column index', () => {
      expect(() => {
        store.set(removeCardFromTableauColumn, { columnIndex: 7, cardId: 'test' });
      }).toThrow('Invalid column index: 7');
    ...
    it('should throw error when card not found', () => {
      ...
      expect(() => {
        store.set(removeCardFromTableauColumn, { columnIndex: 0, cardId: 'non-existent' });
  ...
  describe('Flip Card Action', () => {
    ...
    it('should throw error when card not found', () => {
      ...
      expect(() => {
        store.set(flipCardInTableau, { columnIndex: 0, cardId: 'non-existent' });
      }).toThrow('Card with id non-existent not found in column 0');
    });
  });
  ...
  describe('State Validation', () => {
    ...
    it('should handle complex tableau state', () => {
      const complexTableau = [
        ...
        [],
        [createCard('spades', 4, true), createCard('hearts', 5, false), createCard('clubs', 6, true)],
        [],
